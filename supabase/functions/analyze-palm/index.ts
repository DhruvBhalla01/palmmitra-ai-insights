...
const generatePalmReading = async (imageUrl: string, name: string, age: string, readingType: string, apiKey: string) => {
...
serve(async (req) => {
  ...
  try {
    ...
    if (!imageUrl || !name || !age) {
      return new Response(
        JSON.stringify({ error: 'Missing required fields: imageUrl, name, or age' }),
    ...
    if (!validation.is_palm || validation.confidence < 70) {
      ...
      return new Response(
        JSON.stringify({
          ...
          validated: false,
          validation,
          message: validation.reason || 'This does not appear to be a clear palm photo. Please upload a front-facing palm image with visible palm lines.'
        }),
