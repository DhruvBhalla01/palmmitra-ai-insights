...
The SDK supports TypeScript MCP servers on [either major](/docs/mcp-analytics/sdk-v2.md). The wizard installs the package and configures `instrument()`. It also supports [LLM coding agents](/blog/envoy-wizard-llm-agent.md), such as Cursor and Bolt:
...
Intent, model capture, conversation IDs, and exception capture are on by default. Missing-capability reporting and agent feedback are opt-in. [Python](/docs/mcp-analytics/installation.md#python) uses the same defaults.
